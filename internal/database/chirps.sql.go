...
const truncateChirpsTable = `-- name: TruncateChirpsTable :exec
TRUNCATE TABLE chirps CASCADE
`

func (q *Queries) TruncateChirpsTable(ctx context.Context) error {
	_, err := q.db.ExecContext(ctx, truncateChirpsTable)
	return err
}
